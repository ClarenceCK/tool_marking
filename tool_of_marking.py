...
class Ui_Tool_of_Marking(object):
    ...
    def Magnify(self):
        ...
        pix_map =  pic       
        a = pix_map.width()*1.2
        b = pix_map.height()*1.2
        scaredPixmap = pix_map.scaled(QSize(a, b), aspectRatioMode = Qt.KeepAspectRatio) 
        self.Display.setPixmap(pix_map)
        self.Display = self.Display.setPixmap(scaredPixmap)
        #self.Display.resize(pix_map.width(),pix_map.height())
        pic = pix_map

    def Shrink(self):
        #if self.picture.isNull():   # 没图片,则不执行任何操作
        #    return  
#        martix =QMatrix()        # PyQt4,PyQt5中已废弃
        transform = QTransform()  # PyQt5
#        martix.scale(2,2)        # PyQt4,PyQt5中已废弃
        transform.scale(0.8,0.8)  # PyQt5
        self.Display=self.Displaye.transformed(transform);             # 相应的matrix改为transform
        self.Display.setPixmap(QPixmap.fromImage(self.Display))        # 显示图片到Qlabel控件
        self.resize(self.Display.width(),self.Display.height())
    ...
    def show_img_in_lable_center(self):
        ...
        #若图片宽高都小于label宽高，则按图片原始大小显示
            w = img_w
            h = img_h
        self.Display.setPixmap(pix_map.scaled(w, h))
